...
},
{
	...
	function blockFood(setFood){
		...
		const smallPortions = item.ingredients.map(item => item.name + " - " + item.weightL + "г").join(", ");
		const largePortions = item.ingredients.map(item => item.name + " - " + item.weightM + "г").join(", ");	
		// console.log(item.cost.little);
		...
		<div class="item">
			...
			<hr>
			<p> <b>Ігрідієнти:</b> ${largePortions} </p>
			
			<div class="costItem">
			<button class="btn1" data-name="Item1${item.itemId}"> Little:${item.cost.little}грн</button>
			<button class="btn2" data-name="Item2${item.itemId}"> Maximum:${item.cost.max}грн</button>
			</div>
		</div>
		`
	...
	// Кнопка для ціни в 170грн
const mainObject = {};
let totalCost = 0; // Окремо зберігаємо суму всіх little

function firstBtn(arr) {
    document.querySelectorAll(".btn1").forEach((button, index) => {
        button.addEventListener("click", () => {
            const { name, ingerL, cost } = arr[index]; // Отримуємо cost
            const littleCost = cost.little; // Дістаємо значення little
            const buttonDataName = button.getAttribute('data-name');

            // Додаємо в mainObject значення інгредієнтів
            Object.entries(ingerL).forEach(([key, value]) => {
                mainObject[key] = (mainObject[key] || 0) + value;
            });

            // Додаємо до загальної вартості
            totalCost += littleCost;

            console.log("Поточний стан об'єкта:", mainObject);
            console.log("Загальна вартість:", totalCost);
            console.log(buttonDataName, ingerL);

            // Оновлення списку замовлень
            orderArr.push(`${name} L`);
            inSave.value = orderArr.join(", ");
...
firstBtn(foodItems);
	...
	// Кнопка для ціни в 200грн
function secondBtn(arr){
		const maxCost = document.querySelectorAll(".btn2");
		maxCost.forEach((button, index) => {
			button.addEventListener("click", () => {
            const { name, ingerL, cost } = arr[index]; // Отримуємо cost
            const littleCost = cost.max; // Дістаємо значення little
            const buttonDataName = button.getAttribute('data-name');
...
secondBtn(foodItems);
	...
	// Функція виводу локального масиву localArr на екран за допомогою Div
	function updateSave() {
		const addedFood = document.getElementById('addedFood');
		if(addedFood){
		addedFood.innerHTML = `<p> Food Items: ${JSON.stringify(orderArr)}</p>`;
		}
		
		const myFood = document.getElementById('myFood');
		if(myFood){
		const ourResult = JSON.parse(localStorage.getItem("localArr"));;
		const sliceResult = ourResult.map(subArray => subArray.slice(0, -2));
		console.log(sliceResult);
		
		ourResult.forEach((item, index) => {
			myFood.innerHTML = `All Items: ${JSON.stringify(sliceResult)}`;
		});
		// myFood.innerHTML = `<p> All Items: ${JSON.stringify(localArr)}</p>`;
		}
	}
	updateSave();	
	...
	saveBtn.addEventListener("click", () => {
			const allItems = inSave.value;
			if (allItems) {
				// Додаємо новий підмасив (наприклад, розбиваємо дані за комами)
				const newSubArray = allItems.split(',').map(item => item.trim());
			    ...
			    const timeNow = dateMoment.toLocaleString('uk-UA', options);
				const convertToString = [newSubArray ]+", "+[timeNow];
				const splitConvert = convertToString.split(',');
				

				
				localArr.push(splitConvert);
	...
	saveArrbtn.addEventListener('click', () => {
		const allIngred = Object.entries(getAllMyIngrid()).sort((a, b) => a[1] - b[1]); // Отримуємо масив ключ-значення
		// console.log(mainObject);
		for (const [key, value] of allIngred) {
			// console.log([key, value]);
			const numericValue = Number(value); // Перетворюємо значення в число
			if (isNaN(numericValue) || numericValue <= 0) {
				console.warn(`Інгредієнт "${key}" має некоректне значення: ${value}. Оновлення зупинено.`);
				alert(`Неможливо оновити! Інгредієнт "${key}" має 0 або від’ємне значення.`);
				return [key, value]; // Зупиняємо виконання функції
			}
		}

		// Якщо всі значення > 0, оновлюємо localStorage
		subtractIngredients(allIngredients2, [mainObject]);
...
function renderTablesFromArray(array) {
	...
	// видалити та розкоментувати щоб зробити відображеня сторінки по натисканні на кнопку
	renderTablesFromArray(localArr);
...
// const itemCost = [foodItems[0].ingerL];
